...
- 밸런스게임. 고른 쪽에만 조건 누적, 반대쪽 안 건드림, 리셋 없음. 강도 = 판 번호(1판 맨몸, 강도 2부터).
  - **⚠️ v3.1 가변 길이(2026-07-12, 계획 확정 · 코드 미반영)**: 10판 고정 → **판 수 5~10, 덱마다 자유**(조건 4~9개). 문서는 갱신됨. **엔진(`ROUNDS=10`·decode 정규식·`EXTREME_HOLD=8`)·`decks.test.ts`(길이 9 강제)·플레이 화면은 아직 10판 하드코딩** → 구현 TODO(§7).
...
## 7.1 v3.1 가변 길이 — 코드 할 일 (2026-07-12, 계획 확정 · 코드 미반영)

방향 = **판 수 5~10, 덱마다 자유**(조건 4~9개). 현재 코드는 10판·9조건 하드코딩 → 갭:

- **`engine.ts`**
  - `ROUNDS = 10` 상수 → **덱별 판 수**로. `roundsOf(deck) = deck.a.penalties.length + 1` (맨몸 1판 + 조건 수). 양편 길이 같음을 전제(가드 추가).
  - `decodeChoices` 정규식 `/^[01]{10}$/` → **가변 길이**(`/^[01]{5,10}$/` 등) + 덱 판 수와 일치 검증. `encode/decode` 공유 코덱이 길이 안 박히게.
  - `EXTREME_HOLD = 8`(고정) → **덱 최대 강도 상대값**. 예: `extremeThreshold(deck) = maxStrength(deck) - 1`(마지막 2단계 버팀=극단). `pickResultCard`가 이걸 쓰게.
  - `computeResult`는 `choices.length` 기반이라 대체로 가변 대응되나, "길이 10 가정" 주석·게이지 분모(`/ROUNDS`) 점검.
- **`decks.test.ts`**: `penalties.toHaveLength(9)`·`strengths [2..10]` 강제 → **길이 4~9 허용 + 강도 [2..N] 연속**·**양편 길이 동일**로 테스트 완화/재작성.
- **플레이 화면(`g/[deck]/+page.svelte`)**: `done = choices.length >= ROUNDS`·`Array(ROUNDS)`·게이지 `holdMax/ROUNDS` → **덱 판 수 기준**으로.
- **콘텐츠**: 기존 10판 덱은 그대로 두고, 신규/재저작 덱부터 짧게 끊어도 됨.

**착수 순서**: 엔진 `roundsOf`/임계 함수화 → 테스트 완화 → 플레이 화면 → 신규 짧은 덱 1개로 검증.
