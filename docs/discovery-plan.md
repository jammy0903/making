...
- [x] **유튜브 발원지 채널 ④** (2026-07-16) — 밈은 영상에서 태어난다 → 제목에서 발굴
  ...
  - ✅ 모니터링 채널 **21개**(`streamers.js`, 채널ID 검증 등록) — **제목 밈밀도 실측 후 큐레이션**:
    - 게임/실황: 침착맨·우왁굳·악어·잠뜰(잼민이 대표)·감스트·랄로
    - 먹방: 쯔양·문복희·홍사운드·히밥·입짧은햇님 / 예능: 워크맨·채널십오야·뜬뜬·피식대학·문명특급
    - 밈-대사체: 너덜트·총몇명·피지컬갤러리·빵송국·스낵타운
    - **제거(노이즈)**: 풍월량(게임명 제목)·허팝(설명형 실험)·도티(제네릭 가챠게임) — 제목에서 새 밈 안 나옴
    - 실측: **테토남/테토녀**(피식대학)·눕프로해커(우왁굳)·자만추(너덜트) 등 실시간 밈 포착. quota ~40u/일
