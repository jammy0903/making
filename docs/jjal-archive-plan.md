...
- **짤 클릭 시**: 확대 + **복사·다운로드 버튼** + 출처 링크. 연결된 밈이 있으면 `/m/{id}`로 가는 링크.

### 메신저 사용 흐름 (핵심 유스케이스)

이 페이지의 1차 용도는 **메신저(카톡) 대화 중 상황에 맞는 짤을 확보해 붙여넣는 것**이다.
폰이든 PC든 "짤 찾기 → 내 기기에 확보 → 메신저에 붙여넣기"가 15초 안에 끝나야 한다.
전송은 메신저에서 사용자가 직접 한다 — 우리는 **복사와 다운로드만 확실하게** 만든다.

1. **이미지 복사** — Clipboard API(`ClipboardItem`)로 이미지 자체를 클립보드에 넣는다.
   PC 카톡은 Ctrl+V, 모바일도 채팅 입력창 붙여넣기로 바로 들어간다.
   클립보드는 PNG만 받는 브라우저가 많으므로 JPG 등은 캔버스로 PNG 변환 후 복사.
   **GIF는 복사하면 정지 이미지가 되므로**, GIF엔 복사 대신 다운로드를 기본 버튼으로 보여준다.
2. **이미지 다운로드** — blob 다운로드로 원본 그대로 저장. 폰에서는 갤러리(사진)로,
   PC에서는 파일로 떨어지고, 메신저의 사진 첨부로 올린다. GIF도 움직임 그대로 보존된다.
3. **PWA 홈 화면 추가** — manifest + 아이콘을 갖춰 홈 화면에 앱처럼 꽂는다.
   카톡 ↔ 짤 검색 전환이 2탭이 되어야 "자주 쓰는 도구"가 된다.

`external` 소스(외부 CDN 직링크)는 CORS 때문에 fetch가 막혀 복사·blob 다운로드가
실패할 수 있다. 이 경우 새 탭으로 원본을 열어 길게 눌러 저장(모바일)/우클릭 저장(PC)하도록
안내한다. internal·meme 소스(우리 도메인)는 항상 둘 다 된다.

**안 하기로 한 것**: 카카오링크 공유(SDK)·카톡 챗봇 채널·Web Share 공유 버튼.
전송은 사용자가 메신저에서 직접 하는 게 목적에 맞고, 비용·번거로움 대비 불필요하다.
퍼뜨리기용 기능이 아니라 개인 도구다.
...
- **Phase 2 — 검색 페이지**: `/jjal` 기본 그리드 + 벡터 검색. 밴드·섹션 임계값을 실측으로 확정.
  **메신저 흐름(이미지 복사·다운로드·PWA manifest)도 여기 포함** — 6장의 핵심 유스케이스라 뒤로 미루지 않는다.
