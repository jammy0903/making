...
- **짤 클릭 시**: 확대 + 이미지 복사/다운로드 + 출처 링크. 연결된 밈이 있으면 `/m/{id}`로 가는 링크.
  여기에 **"보내기" 버튼**(아래 참고)을 나란히 둔다.

### 메신저 사용 흐름 (핵심 유스케이스)

이 페이지의 1차 용도는 **메신저(카톡) 대화 중 상황에 맞는 짤을 찾아 바로 보내는 것**이다.
"대화 중 짤 찾기 → 전송"이 15초 안에 끝나야 한다.

1. **Web Share API 이미지 공유** — "보내기" 버튼이 `navigator.share({ files: [이미지] })`로
   OS 공유 시트를 띄운다 → 카톡 선택 → 방 선택 → 이미지로 바로 전송.
   `public/share.js`의 투표 카드와 같은 패턴(Web Share + 다운로드 폴백)이다.
   `external` 소스는 CORS로 fetch→File 변환이 막힐 수 있으니 그 경우 링크 공유로 폴백.
2. **PWA 홈 화면 추가** — manifest + 아이콘을 갖춰 홈 화면에 앱처럼 꽂는다.
   카톡 ↔ 짤 검색 전환이 2탭이 되어야 "자주 쓰는 도구"가 된다.
3. **클립보드 복사** — 데스크톱 카톡용. 이미지를 클립보드에 복사해 Ctrl+V로 붙여넣는다.

**안 하기로 한 것**: 카카오링크 공유(SDK)·카톡 챗봇 채널. 비용·등록 절차 대비 목적
(개인이 적재적소에 짤 쓰기)에 불필요하다. 퍼뜨리기용 기능이 아니라 개인 도구다.
...
- **Phase 2 — 검색 페이지**: `/jjal` 기본 그리드 + 벡터 검색. 밴드·섹션 임계값을 실측으로 확정.
  **메신저 흐름(Web Share 보내기·클립보드 복사·PWA manifest)도 여기 포함** — 6장의 핵심 유스케이스라 뒤로 미루지 않는다.
