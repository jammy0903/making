...
export const load: PageServerLoad = async ({ params, fetch, setHeaders }) => {
  ...
  type Nb = { id: number; name: string };
  const [prevRows, nextRows, comments] = await Promise.all([
    sbGet<Nb[]>(fetch, `meme_cards?id=lt.${id}&select=id,name&order=id.desc&limit=1`).catch(() => [] as Nb[]),
    sbGet<Nb[]>(fetch, `meme_cards?id=gt.${id}&select=id,name&order=id.asc&limit=1`).catch(() => [] as Nb[]),
    loadComments(fetch, id).catch(() => []),
  ]);
  setHeaders({ 'cache-control': 'public, max-age=0, s-maxage=120' });
  return { meme: mapCard(rows[0]), comments, prev: prevRows[0] ?? null, next: nextRows[0] ?? null };
};
